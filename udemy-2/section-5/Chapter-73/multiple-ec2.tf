...
resource "aws_instance" "example1" {
  ...
  volume_tags = {
    ...
    Environment : "poc"
    AppName : "pulsar-testing-client-${each.value.zone}-ebs"
    InfraOwner : "sre-cloud-reliability@tavisca.com"
    BusinessUnit : "travel.app"
    Backup : "no"
    Product : "poap"
    Name : "pulsar-testing-client--${each.value.zone}-ebs"
  }
  tags = {
    DataClassification : "restricted"
    Environment : "poc"
    AppName : "pulsar-testing-client-error-${each.value.zone}"
    InfraOwner : "sre-cloud-reliability@tavisca.com"
    BusinessUnit : "travel.app"
    Backup : "no"
    Product : "poap"
    Name : "pulsar-testing-client-error-${each.value.zone}"
